...
# dataset in /.../NREL_works/datasets_conductance #
...
#filename = "adj_matrix_" + str(1) + ".csv"
summary_result_gml_version = np.zeros((101,))
weight_rank_M = np.zeros((101,13))

for kk in range(101):
    filename = "graph_sequence_" + str(kk) +".gml"
    tmp_g = igraph.read(filename)
    A = np.array(tmp_g.get_adjacency().data)
    weight_mat = np.array(tmp_g.get_adjacency("weight").data)
    ...
    for ii in motif_type:
        if np.sum(Motif_Adj_f(A, ii)) != 0:
            ...
            conductances = np.divide(np.cumsum(C_sums - 2 * np.sum(np.tril(C), axis=1)),
                                     np.minimum(volumes, volumes_other))
            ...
            min_value_sum_mat[int("".join(filter(str.isdigit, ii))) - 1] = 1  # 'nan'


    weight_rank_M[kk,:] = np.argsort(min_value_sum_mat)



    if len(list(np.where(min_value_sum_mat == np.min(min_value_sum_mat)))[0])>1:
        summary_result_gml_version[kk] = list(np.where(min_value_sum_mat == np.min(min_value_sum_mat)))[0][0]
    else:
        summary_result_gml_version[kk] = list(np.where(min_value_sum_mat == np.min(min_value_sum_mat)))[0]


summary_result_df_gml_version = pd.DataFrame(summary_result_gml_version)
summary_result_df_gml_version.to_csv('summary_result_gml_version.csv',index=False)
weight_rank_M_df = pd.DataFrame(weight_rank_M, columns=["rank_"+ str(i) for i in range(13)])
weight_rank_M_df.to_csv('weight_rank_M_dataframe.csv',index=True)

'''
conductances_sum_mat_df = pd.DataFrame(conductances_sum_mat)
print(conductances_sum_mat)
min_value_sum_mat_df = pd.DataFrame(min_value_sum_mat)
print(min_value_sum_mat)
conductances_sum_mat_df.to_csv('conductances_case118.csv',index=False)
min_value_sum_mat_df.to_csv('min_value_sum_mat_case118.csv',index=False)
'''

'''
# below R script is used to generated simplifid graph from output_graph_under_attack_f function (.gml generate from R) #
for (kk in c(2:102)) {
